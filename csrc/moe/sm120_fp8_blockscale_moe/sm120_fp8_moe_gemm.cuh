...
// Adapted from TensorRT-LLM SM120 blockscale MOE GEMM kernel.
//
// SM120BlockScaledMoeKernel: Persistent-thread grouped GEMM kernel for
// SM120 FP8 block-scale MOE. Uses per-expert TMA addressing via
// domain_offset and a custom scheduler for work distribution.
//
// Data layout:
//   A:   [M_total, K]                  — 2D, TMA L=1
//   B:   [num_experts, N, K]           — 3D, TMA L=num_experts
//   SFA: [sf_K, pad(M_total,4)]        — 2D, TMA L=1 (padded for alignment)
//   SFB: [num_experts, pad(N,4), sf_K] — 3D, TMA L=num_experts
//   D:   [M_total, N]                  — 2D, TMA L=1
...
// MOE Scheduler: distributes work across expert groups using persistent
// thread blocks. Uses element-level offsets (token_offset) for variable-size
// expert groups.
...
struct SM120BlockScaledMoeScheduler {
  ...
  __device__ __forceinline__ bool get_next_block(int32_t& m_block_idx,
                                                 int32_t& n_block_idx) {
    auto next_block_idx = ++current_iter * gridDim.x + blockIdx.x;
    while (true) {
      if (curr_group_idx >= num_groups) return false;
      ...
      if (next_block_idx < next_m_cumsum * num_n_blocks) break;
  ...
  __device__ __forceinline__ int32_t get_m_offset() const { return m_offset; }
...
// SM120 Block-Scaled MOE GEMM Kernel
...
struct SM120BlockScaledMoeKernel {
  ...
  static constexpr Params to_underlying_arguments(
    ...
    auto tensor_A = make_tensor(make_gmem_ptr(args.ptr_A),
                                make_layout(make_shape(M, K, 1), args.dA));
    typename KT::TMA_A tma_load_a = make_tma_copy(
        SM90_TMA_LOAD{}, tensor_A, typename KT::SmemLayoutA{}(_, _, Int<0>{}),
        make_shape(shape<0>(typename KT::TileShape{}),
                   shape<2>(typename KT::TileShape{})),
        _1{});

    // B TMA: (N, K, num_experts) — B is 3D with experts as batch
    auto tensor_B =
        make_tensor(make_gmem_ptr(args.ptr_B),
                    make_layout(make_shape(N, K, num_experts), args.dB));
    typename KT::TMA_B tma_load_b = make_tma_copy(
        SM90_TMA_LOAD{}, tensor_B, typename KT::SmemLayoutB{}(_, _, Int<0>{}),
        make_shape(shape<1>(typename KT::TileShape{}),
                   shape<2>(typename KT::TileShape{})),
        _1{});

    // SFA TMA: uses m_padded to match quantization kernel's scale layout
    ...
    typename KT::TMA_SFA tma_load_sfa = make_tma_copy(
        SM90_TMA_LOAD{}, tensor_sfa,
        typename KT::SmemLayoutSFA{}(_, _, Int<0>{}),
        make_shape(shape<0>(typename KT::ScaleTileShape{}),
                   shape<2>(typename KT::ScaleTileShape{})),
        _1{});
    ...
    typename KT::TMA_SFB tma_load_sfb = make_tma_copy(
        SM90_TMA_LOAD{}, tensor_sfb,
        typename KT::SmemLayoutSFB{}(_, _, Int<0>{}),
        make_shape(shape<1>(typename KT::ScaleTileShape{}),
                   shape<2>(typename KT::ScaleTileShape{})),
        _1{});

    return {tma_load_a, tma_load_b, tma_load_sfa,   tma_load_sfb,
            args.ptr_D, M,          N,               K,
            num_experts, args.token_offset};
  }
  ...
  static auto get_mbarriers(SharedStorage& shared_storage) {
    auto* ab_full_mbar =
        recast_ptr<FullBarrier>(&shared_storage.barriers.ab_full_mbar[0]);
    auto* ab_empty_mbar =
        recast_ptr<EmptyBarrier>(&shared_storage.barriers.ab_empty_mbar[0]);
    auto* sf_full_mbar =
        recast_ptr<FullBarrier>(&shared_storage.barriers.sf_full_mbar[0]);
    auto* sf_empty_mbar =
        recast_ptr<EmptyBarrier>(&shared_storage.barriers.sf_empty_mbar[0]);
    auto* store_full_mbar =
        recast_ptr<EmptyBarrier>(&shared_storage.barriers.store_full_mbar[0]);
    auto* store_empty_mbar =
        recast_ptr<EmptyBarrier>(&shared_storage.barriers.store_empty_mbar[0]);
    return cute::make_tuple(ab_full_mbar, ab_empty_mbar, sf_full_mbar,
                            sf_empty_mbar, store_full_mbar, store_empty_mbar);
  }

  // Load scale factors (SFA, SFB) via TMA
  CUTE_DEVICE
  static void load_sf(Params const& params, SharedStorage& shared_storage,
                      int32_t m_offset, int32_t m_block_idx,
                      int32_t n_block_idx, int32_t expert_idx,
                      int32_t sf_tile_count, uint32_t& sf_phase,
    ...
    auto mSFA_full = params.tma_load_sfa.get_tma_tensor(
        shape(KT::deduce_sfa_layout(sfa_shape)));
    int32_t sf_m_offset = compute_padded_offset(m_offset, expert_idx);
    auto mSFA_mkl = cute::domain_offset(make_coord(sf_m_offset, 0, 0),
                                        mSFA_full);

    auto sfb_shape =
        make_shape(params.M, params.N, params.K, params.num_experts);
    auto mSFB_nkl = params.tma_load_sfb.get_tma_tensor(
        shape(KT::deduce_sfb_layout(sfb_shape)));

    auto gSFA_mkl = local_tile(mSFA_mkl, typename KT::ScaleTileShape{},
                               make_coord(_, _, _), Step<_1, X, _1>{});
    auto gSFB_nkl = local_tile(mSFB_nkl, typename KT::ScaleTileShape{},
    ...
    for (int32_t sf_tile_idx = 0; sf_tile_idx < sf_tile_count;
         ++sf_tile_idx) {
      ...
      cute::copy(tma_copy_sfa, tAgSFA(_, _, _, sf_tile_idx),
                 tAsSFA(_, _, _, Int<0>{}));
      auto tma_copy_sfb = params.tma_load_sfb.with(
          *recast_ptr<ProducerBarrierType>(&sf_full_barrier));
      cute::copy(tma_copy_sfb, tBgSFB(_, _, _, sf_tile_idx),
                 tBsSFB(_, _, _, Int<0>{}));
  ...
  // Load A and B data via TMA
  CUTE_DEVICE
  static void load_ab(Params const& params, SharedStorage& shared_storage,
                      int32_t m_offset, int32_t m_block_idx,
                      int32_t n_block_idx, int32_t expert_idx,
                      int32_t sf_tile_count, uint32_t& ab_phase,
                      uint32_t& store_phase) {
    using X = Underscore;

    auto mA_full =
        params.tma_load_a.get_tma_tensor(make_shape(params.M, params.K, 1));
    auto mA_mkl = cute::domain_offset(make_coord(m_offset, 0, 0), mA_full);

    auto mB_nkl = params.tma_load_b.get_tma_tensor(
        make_shape(params.N, params.K, params.num_experts));

    auto gA_mkl = local_tile(mA_mkl, typename KT::TileShape{},
                             make_coord(_, _, _), Step<_1, X, _1>{});
    auto gB_nkl = local_tile(mB_nkl, typename KT::TileShape{},
                             make_coord(_, _, _), Step<X, _1, _1>{});
    ...
    for (int32_t k_tile_idx = 0; k_tile_idx < k_tile_count;
      ...
      cute::for_each(
          ...
          [&](auto write_stage) {
            ...
            cute::copy(tma_copy_a,
                       tAgA(_, _, _, k_tile_idx + write_stage),
            ...
            ab_full_mbar[write_stage].arrive_and_expect_tx(
                KT::TmaABTransactionBytes);
          });
      ab_phase ^= 1;
    }
  }

  // MMA computation: consumes data from shared memory, produces accum,
  // then writes result to smem and gmem
  CUTE_DEVICE
  static void mma(Params const& params, SharedStorage& shared_storage,
                  int32_t sf_tile_count, int32_t m_offset,
                  int32_t m_boundary, int32_t m_block_idx,
                  int32_t n_block_idx, uint32_t& sf_phase,
                  uint32_t& ab_phase) {
    ...
    typename KT::TiledMma mma_op;
    auto tile_shape_mnk = tile_shape(mma_op);
    auto thr_mma = mma_op.get_thread_slice(thread_idx);
    auto accum = partition_fragment_C(
        mma_op, cute::take<0, 2>(typename KT::TileShape{}));
    auto tCrA = thr_mma.partition_fragment_A(sA(_, _, Int<0>{}));
    auto tCrB = thr_mma.partition_fragment_B(sB(_, _, Int<0>{}));

    // A smem → reg
    auto s2r_copy_A = make_tiled_copy_A(typename KT::SmemCopyAtomA{}, mma_op);
    ...
    // B smem → reg
    auto s2r_copy_B = make_tiled_copy_B(typename KT::SmemCopyAtomB{}, mma_op);
    ...
    auto s2r_copy_SFA = make_tiled_copy_impl(
        typename KT::SmemCopyAtomSF{}, KT::get_layoutSFA_TV(mma_op),
        make_shape(size<0>(tile_shape(mma_op)), _1{}));
    ...
    auto s2r_copy_SFB = make_tiled_copy_impl(
        typename KT::SmemCopyAtomSF{}, KT::get_layoutSFB_TV(mma_op),
        make_shape(size<1>(tile_shape(mma_op)), _1{}));
    ...
    for (int32_t sf_tile_idx = 0; sf_tile_idx < sf_tile_count - 1;
         ++sf_tile_idx) {
      ...
      cute::for_each(
          cute::make_int_sequence<KT::kNumStagePerSF>{}, [&](auto iter) {
            cute::for_each(
                cute::make_int_sequence<KT::AB_Stages>{},
                [&](auto read_stage) {
                  ...
                  auto tCrSFA_stage =
                      tCrSFA_frg(_, _, _, iter * KT::AB_Stages + read_stage);
                  auto tCrSFB_stage =
                      tCrSFB_frg(_, _, _, iter * KT::AB_Stages + read_stage);
                  cute::gemm(mma_op,
                             make_zip_tensor(tCrA, tCrSFA_stage),
                             make_zip_tensor(tCrB, tCrSFB_stage), accum);
                });
            ab_phase ^= 1;
          });
      sf_phase ^= 1;
        ...
        cute::make_int_sequence<KT::kNumStagePerSF>{}, [&](auto iter) {
          cute::for_each(
              ...
              [&](auto read_stage) {
                ...
                cute::copy(s2r_copy_B, tXsB(_, _, _, read_stage), tXrB);
                ab_empty_mbar[read_stage].arrive();
                if constexpr (iter == KT::kNumStagePerSF - 1 &&
    ...
    // Epilogue: convert accum to output type and write to smem, then gmem
    ...
    cutlass::NumericArrayConverter<typename KT::ElementD,
                                   typename KT::ElementAccum, 2>
        converter;
    cute::for_each(cute::make_int_sequence<cute::size(epi_frg)>{},
                   [&](auto i) { epi_frg(i) = converter(accum_frg(i)); });

    auto sD_ = cute::make_tensor(
        cute::make_smem_ptr(shared_storage.tensors.store.smem_O.begin()),
        typename KT::SmemLayoutO{});
    auto sD = as_position_independent_swizzle_tensor(sD_);

    // Copy rf → smem
    auto tiled_copy_R2S =
        cute::make_tiled_copy_C(typename KT::SmemCopyAtomR2S{}, mma_op);
    ...
    // Copy rf → gmem with bounds checking
    ...
    for (int m = 0; m < cute::size<1>(tRG_gD); ++m) {
      ...
      for (int n = 0; n < cute::size<2>(tRG_gD); ++n) {
        if (cute::get<0>(tRG_cD(0, m, n)) < residue_m &&
            cute::get<1>(tRG_cD(0, m, n)) < residue_n) {
          cute::copy(typename KT::GmemCopyAtomR2G{}, tRG_rD(cute::_, m, n),
                     tRG_gD(cute::_, m, n));
        }
      }
    }
...
#pragma unroll
    ...
    if (warp_idx >= KT::kNumMathWarps) {
      ...
      if (warp_idx == ab_warp_idx) {
        ...
        if (lane_predicate) {
          Scheduler scheduler(params.N, params.num_experts,
                              params.token_offset);
          int32_t m_block_idx_local, n_block_idx_local;
          while (scheduler.get_next_block(m_block_idx_local,
                                          n_block_idx_local)) {
            load_ab(params, shared_storage, scheduler.get_m_offset(),
                    m_block_idx_local, n_block_idx_local,
                    scheduler.get_expert_idx(), sf_tile_count, ab_phase,
                    store_phase);
          }
        }
        __syncwarp();
      ...
      if (warp_idx == sf_warp_idx) {
        ...
        if (lane_predicate) {
          ...
          while (scheduler.get_next_block(m_block_idx_local,
                                          n_block_idx_local)) {
            load_sf(params, shared_storage, scheduler.get_m_offset(),
                    m_block_idx_local, n_block_idx_local,
                    scheduler.get_expert_idx(), sf_tile_count, sf_phase,
                    store_phase);
          }
        }
        __syncwarp();
      ...
      // Math warps: perform MMA and write output
      uint32_t sf_phase = 0;
      uint32_t ab_phase = 0;
      Scheduler scheduler(params.N, params.num_experts,
                          params.token_offset);
      ...
      while (scheduler.get_next_block(m_block_idx_local,
                                      n_block_idx_local)) {
        mma(params, shared_storage, sf_tile_count,
            scheduler.get_m_offset(), scheduler.get_m_boundary(),
            m_block_idx_local, n_block_idx_local, sf_phase, ab_phase);
      }
    }
  }
};
