        ...
        for layer_name in kv_cache_group_spec.layer_names:
            ...
            if isinstance(kv_cache_spec, AttentionSpec):
                ...
                if kv_cache_spec.page_size_padded is not None:
                    # MLA path: use storage_block_size with strided view.
                    blocks_for_shape = num_blocks
                    block_size_for_shape = kv_cache_spec.storage_block_size
                else:
                    # Regular attention: virtual block splitting via
                    # kernel_block_size for backend compatibility.
                    num_blocks_per_kv_block = (
                        kv_cache_spec.block_size // kernel_block_size
                    )
                    blocks_for_shape = num_blocks * num_blocks_per_kv_block
                    block_size_for_shape = kernel_block_size

                attn_backend = attn_backends[layer_name]
                kv_cache_shape = attn_backend.get_kv_cache_shape(
                    blocks_for_shape,
                    block_size_for_shape,
                ...
                raw_typed = raw_tensor.view(dtype)
                if kv_cache_spec.page_size_padded is not None:
                    ...
                    kv_cache = raw_typed.view(kv_cache_shape)
...
def init_kv_cache(
    ...
    if attn_groups is not None:
        ...
        # Fallback: use each group's block_size (no virtual block splitting).
        kernel_block_sizes = [
